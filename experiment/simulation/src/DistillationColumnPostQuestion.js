...
function DistillationColumnPostQuestion()
{
	$("#Selection").html("");
	...
	for (var i = 0; i < boilerHeatExchangerPostJson.data["SEC"].length; i++) {

		for (var j = 0; j < boilerHeatExchangerPostJson.data["SEC"][0].QUES.length; j++) {

			questions += '<div class="col-md-12">'
				+ '<br><span class="queno">Question No : ' + (i + 1) + '</span>'
				+ '<br><div class="queTitle">'

				+ boilerHeatExchangerPostJson.data["SEC"][i].QUES[j].QC
				+ '</div>'

			for (var k = 0; k < boilerHeatExchangerPostJson.data["SEC"][i].QUES[j].ANS.length; k++) {

				questions += '<div>'
					+ '<div class="ansTitle col-md-6">'

					+ "<input class='radioType'  name='radio-" + i + "' id='" + i + "' ANSID='" + boilerHeatExchangerPostJson.data["SEC"][i].QUES[j].ANS[k].ANSID + "' type='radio'"
					+ "' value='"
					+ boilerHeatExchangerPostJson.data["SEC"][i].QUES[j].ANS[k].content
					+ "' >"
